...
export default function SignatureCollection() {
  ...
  const handleSaveSignature = async () => {
    ...
    // Update the Base64 signature string directly in Supabase
    try {
      // Update the specific person's signature in the Supabase ticket data with the Base64 string
      ...
      // Update the ticket in Supabase with the modified people array
      const { error } = await supabase
        .from('Tickets')
        .update({ data: { ...currTicket.data, people: updatedPeople } })
        .eq('id', id);
